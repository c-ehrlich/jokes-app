...
import { jokes } from "~/server/db/schema";
...
export const jokesRouter = createTRPCRouter({
  create: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db.insert(jokes).values({
        topic: input.topic,
        ...parsed,
      });
    }),
  ...
  getAll: publicProcedure.query(async ({ ctx }) => {
    const jokes = await ctx.db.query.jokes.findMany({
      orderBy: (joke, { desc }) => [desc(joke.createdAt)],
    });
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.delete(jokes).where(eq(jokes.id, input.id));
    }),
});
